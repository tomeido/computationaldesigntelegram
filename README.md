...
**14개 RSS/Atom 출처와 5개 공식 GitHub 저장소의 릴리스**를 살펴보고, 구체적인 제작 방법·연구 내용·기능 변화가 있는 자료만 자동 발행합니다. 오래된 좋은 도구는 봇의 `/tools`에서 별도로 찾아볼 수 있습니다.
...
`/tools`는 운영자가 설정한 저장소 목록을 보여 줍니다. 새 릴리스가 없거나 오래된 도구도 확인할 수 있으며, 이 명령은 AI 번역이나 GitHub API를 호출하지 않습니다. 실제 실행 환경과 라이선스는 연결된 저장소에서 확인하세요.
...
Web3·블록체인과 컴퓨테이셔널 디자인의 접점이 최우선입니다. 제목이나 같은 문장 안에 두 분야의 연결 근거가 있어야 하며, 작가 이력 등에 NFT가 한 번 등장하는 것만으로 Web3 우선순위를 주지 않습니다. Web3·AI·디자인이 겹치는 소식, AI·컴퓨테이셔널 디자인, 일반 컴퓨테이셔널 디자인을 이어서 다룹니다. 일반 코인 시황·가격 예측·에어드롭 홍보는 제외하는 방향으로 필터링합니다.

게시물은 일반 **소식**, **논문**, **투자·지원**, **작품·실험**, **GitHub 업데이트**로 구분합니다. 논문은 생성형·파라메트릭 설계, 기하와 디지털 제작 등 관련 연구를 선별합니다. arXiv 수록 여부만으로 학술 심사 통과를 판단하지 않으며 게재 상태·실험 조건은 원문에서 확인하도록 표시합니다. 투자·지원은 관련 기업과 프로젝트의 자금 조달·인수·지원사업 발표를 다루며 매수·매도 추천을 생성하지 않습니다. 작품·실험은 구현 기법이나 시연이 있는 제작 사례를 다룹니다. 각 종류의 새 소식이 매번 있는 것은 아니며 전체 발행 시각과 회당 최대 5건은 동일합니다.

번역 전에 본문 근거를 검사합니다. 제목만 있는 검색 결과, 내용이 부족한 홍보, 등록 안내만 있는 행사·강좌, 의존성 갱신·오탈자 수정만 있는 릴리스는 자동 발행에서 제외합니다. 논문에는 구체적인 방법이나 평가 설명을, 투자·지원에는 실제 발표와 금액·참여사·신청 조건 등의 근거를 요구합니다. `collect --json`에서 제외 이유를 확인할 수 있습니다. 이 검사는 자료의 완성도를 선별하는 규칙이며 과학적 타당성이나 투자 가치를 검증하지는 않습니다.

GitHub에서는 Art Blocks, onchfs, COMPAS, p5.js, NVIDIA Warp의 공식 릴리스를 수집합니다. 실제 변경 기록의 핵심 문장을 번역하고, 운영자가 작성한 도구의 **활용** 설명은 별도로 표시합니다. 저장소·문서·예제 링크와 GitHub API의 라이선스 표기를 함께 제공하며, 라이선스가 확인되지 않으면 확인 필요로 표시합니다. 저장소 전체의 라이선스 표기는 개별 버전·파일의 이용 조건을 대신하지 않습니다. API 응답은 6시간 캐시하며 별도 GitHub 키는 필요하지 않습니다.

같은 주제 우선순위 안에서는 소식 종류를 번갈아 선택합니다. 논문 후보가 많아도 작품이나 투자·지원 소식이 함께 있다면 순서에 반영하며, Web3 → AI → 일반 디자인의 우선순위는 유지합니다.
...
원문 전체 분석, 논문 실험 재현, 코드 실행 검증은 하지 않습니다. RSS 본문·논문 초록·arXiv 저자 코멘트에 실제로 적힌 코드·데모·프로젝트 링크는 최대 3개 보존하고 **원문 링크**로 표시합니다. 논문 제목으로 저장소를 추측하거나 저자 공식 코드로 단정하지 않습니다. 날짜가 없거나 기본 7일보다 오래된 항목은 자동 게시 후보에서 제외하고, 제목만 확보한 항목은 번역·게시하지 않습니다. 오래된 도구는 `/tools`에서 확인하세요. 수집원과 추가 방법은 [수집원 안내](docs/sources.md)를 참고하세요.
...
| `collect` / `collect --json` | RSS·GitHub 수집, 품질 통과 후보와 제외 이유 확인, 번역·게시 없음 |
...
| `REPOSITORIES_FILE` | `config/repositories.json` | GitHub 릴리스·`/tools` 목록 설정, 빈 값이면 둘 다 끔 |
...
GitHub 목록은 `config/repositories.json`의 `owner`, `name`, `why`(활용 설명), `topic`(분야 근거), 선택적인 `docs_url`·`example_url`로 관리합니다. 저장소 이름이나 인기도만으로 새 소식을 만들지는 않습니다. `REPOSITORIES_FILE=`로 비우면 RSS 수집은 유지하고 GitHub 릴리스와 `/tools` 목록을 끕니다.

## Docker로 계속 운영하기

Docker와 Docker Compose가 설치된 서버에서 `.env`에 봇 토큰과 번역 방식을 먼저 설정하세요. 아래 구성은 `config/`를 읽기 전용으로 연결하고, **번역·GitHub API 캐시·발송 기록·수신 진행 위치·설치한 로컬 모델**을 `bot-data` 볼륨에 보존합니다. Docker에서는 경로 설정 기본값을 유지하세요. 컨테이너는 **메모리 1,536 MiB·CPU 2개 분량**으로 제한합니다. 로컬 모델 실행 과정에서 약 945 MB의 메모리를 사용한 측정값이 있습니다. Gemini 모드에서는 로컬 모델을 로드하지 않습니다.
