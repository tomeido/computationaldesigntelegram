...
설치 스크립트는 `.venv`를 만들고 `requirements.lock`의 패키지와 로컬 번역 기능을 설치합니다. 기존 `.env`가 없으면 설정 예시를 복사하며, 기존 값은 덮어쓰지 않습니다. `setup-translator`는 [Argos가 패키징한 Meta M2M100 418M 모델](https://data.argosopentech.com/argospm/v2/translate-fairseq_m2m_100_418M.argosmodel)을 내려받아 기본 `data/models/m2m100`에 설치합니다. 다운로드는 약 **437 MiB**, 설치 후 모델 파일은 약 **494 MB**입니다. `run`은 모델을 자동으로 내려받지 않으므로 먼저 설치하세요.

한국어 브리핑은 출처에서 핵심 문장 1~2개를 골라 번역하는 방식입니다. 설치한 모델을 CPU에서 실행하며, 매번 외부 AI 서비스에 요약이나 번역을 요청하지 않습니다. 뉴스 수집과 텔레그램 통신에는 인터넷 연결이 필요합니다. 원본 모델은 [Meta M2M100 418M](https://huggingface.co/facebook/m2m100_418M)이며 MIT 라이선스입니다. 배포 모델과 실행 환경은 [Argos 안내](https://www.argosopentech.com/), [CTranslate2 문서](https://opennmt.net/CTranslate2/installation.html)를 참고하세요.
...
설정된 RSS/Atom 피드의 제목과 발췌에서 핵심 문장을 고르고, 영어를 로컬 모델로 한국어 기계번역합니다. **번역에 오역이 있거나 발췌에서 맥락이 빠질 수 있습니다.** 원문에 없는 숫자·주소, 관련 없는 안내 문구, 과도한 반복 등 이상 징후를 검사합니다. 본문 번역이 검사에 걸리면 검사를 통과한 제목만 제공하고, 제목도 검사에 걸리면 해당 소식을 건너뜁니다. 이 검사가 번역 정확성을 보장하지는 않습니다.
...
| `preview` | 로컬 한국어 브리핑 미리보기, 게시 없음 |
| `doctor` | 설정·번역 모델·Telegram 연결 확인 |
...
| `LOCAL_MODEL_PATH` | `data/models/m2m100` | 설치한 로컬 번역 모델 경로 |
...
Docker와 Docker Compose가 설치된 서버에서 `.env`를 먼저 작성하세요. 아래 구성은 `config/`를 읽기 전용으로 연결하고, **번역 모델·발송 기록·수신 진행 위치**를 `bot-data` 볼륨에 보존합니다. Docker에서는 경로 설정 기본값을 유지하세요. 컨테이너는 **메모리 1,536 MiB·CPU 2개 분량**으로 제한합니다. 모델 실행 과정에서 약 945 MB의 메모리를 사용한 측정값이 있으므로 다른 서버 작업을 위한 여유 공간도 확보하세요.

```bash
docker compose build

# Docker 볼륨에 번역 모델 설치: 최초 한 번
...
로컬 환경에 이미 모델을 설치했어도 Docker 볼륨은 별도 저장소이므로 볼륨 안에도 모델을 설치해야 합니다. `docker compose down`은 봇을 중지하고 데이터를 보존합니다. `.env`를 변경했다면 `docker compose up -d --force-recreate`로 반영하세요. `docker compose down -v`는 모델과 발송 기록 볼륨까지 삭제합니다.
...
자동 테스트는 외부 응답과 번역기를 대체해 검사합니다. 운영 환경에서는 모델 설치 후 `preview`로 번역 결과를, `doctor`로 텔레그램 연결을 확인하세요. `publish`는 실제 채널 게시입니다.
